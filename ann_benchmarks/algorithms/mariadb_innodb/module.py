...
class MariaDBInnodb(BaseANN):
    ...
    def prepare_options(self):
        self._perf_stat = os.environ.get('PERF', 'no') == 'yes' and MariaDBInnodb.can_run_perf()
        self._perf_record = os.environ.get('FLAMEGRAPH', 'no') == 'yes' and MariaDBInnodb.can_run_flamegraph()
        ...
        user_option = MariaDBInnodb.get_user_option()
    ...
    def can_run_flamegraph():
        if not MariaDBInnodb.can_run_perf():
            return False
    ...
    def fit(self, X):
        ...
        self._cur.execute("select count(*) from t1");
